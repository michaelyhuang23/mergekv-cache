        ...
        for batch in range(comp_key.shape[0]):
            for head in range(comp_key.shape[1]):
                for i in range(separators.shape[-1] + 1):
                    ...
                    merged_attn_score[batch, head, i] = comp_attn_score[batch, head, start:end].sum(dim=-1) 
                    merged_attn_count[batch, head, i] = comp_attn_count[batch, head, start:end].sum(dim=-1) 
                    merged_key[batch, head, i, :] = comp_key[batch, head, start:end, :].mean(dim=-2) 
                    merged_value[batch, head, i, :] = comp_value[batch, head, start:end, :].mean(dim=-2) 
                    merged_id[batch, head, i] = (start+end)//2
...
class HybridKV(Cache):
    def __init__(self, compression_ratio: float, window_ratio: float, top_k_ratio: float, tolerance: float = 0.05) -> None:
        ...
        self.attn_cache: List[torch.Tensor] = []
        self.attn_count: List[torch.Tensor] = []
        self.rel_attn_cache: List[torch.Tensor] = []
        self.rel_attn_count: List[torch.Tensor] = []
        self.compression_ratio = compression_ratio
        self.window_ratio = window_ratio
        self.top_k_ratio = top_k_ratio
        self.tolerance = tolerance
    ...
    def clip(self, layer_idx: int, attn_func, num_heads: int):
        key_cache = self.key_cache[layer_idx]
        w = math.ceil(1 / self.compression_ratio)
        key_cache_tmp = rearrange(key_cache, "b h s d -> (b h) d s")
        #key_cache_tmp = torch.nn.functional.pad(key_cache_tmp, (w, w), mode='zero')
        key_cache_mean = torch.nn.functional.max_pool1d(key_cache_tmp, 2*w+1, padding='same', stride=1)
        key_cache_mean = rearrange(key_cache_mean, "(b h) d s -> b h s d", h=num_heads)
        _, rel_attn_weights = attn_func(key_cache - key_cache_mean)
        _, attn_weights = attn_func(key_cache)
        ...
        rel_attn_weights = rearrange(rel_attn_weights, "b (h gs) s1 s2 -> b h gs s1 s2", h=num_heads)
        rel_aggr_attn_scores = rel_attn_weights.sum(dim=(-2, -3)) # sum over s1 and gs
        if len(self.rel_attn_cache) <= layer_idx:
            self.rel_attn_cache.append(rel_aggr_attn_scores)
            self.rel_attn_count.append(torch.ones_like(rel_aggr_attn_scores) * rel_attn_weights.shape[-2])
        else:
            cur_len = self.rel_attn_cache[layer_idx].shape[-1]
            self.rel_attn_cache[layer_idx] += rel_aggr_attn_scores[..., :cur_len]
            self.rel_attn_count[layer_idx] += torch.ones_like(rel_aggr_attn_scores[..., :cur_len]) * rel_attn_weights.shape[-2]
            self.rel_attn_cache[layer_idx] = torch.cat([self.rel_attn_cache[layer_idx], rel_aggr_attn_scores[..., cur_len:]], dim=-1)
            rel_aggr_attn_counts = (rel_attn_weights[..., cur_len:] > 1e-6).sum(dim=(-2, -3))
            self.rel_attn_count[layer_idx] = torch.cat([self.rel_attn_count[layer_idx], rel_aggr_attn_counts], dim=-1)

        key_cache = self.key_cache[layer_idx]
        value_cache = self.value_cache[layer_idx]
        attn_cache = self.attn_cache[layer_idx]
        attn_count = self.attn_count[layer_idx]
        rel_attn_cache = self.rel_attn_cache[layer_idx]
        rel_attn_count = self.rel_attn_count[layer_idx]
...
def constrained_kmeans(values, separators):
    chunk_size = int(values.shape[0] / (len(separators)+1)) + 1
    for loop in range(chunk_size):
        for i in range(len(separators)):
            ...
            if i == len(separators) - 1:
                ...
                r = separators[i+1] + 1
            if (values[mid : r].mean() - values[separators[i]]).norm() < (values[l : mid].mean() - values[separators[i]]).norm() and separators[i] > l:
                separators[i] -= 1
            elif (values[l : mid-1].mean() - values[separators[i]]).norm() < (values[mid-1 : r].mean() - values[separators[i]]).norm() and separators[i] < r-2:
                separators[i] += 1


class CentroidKV(Cache):
    def __init__(self, compression_ratio: float, window_ratio: float, top_k_ratio: float, tolerance: float = 0.05) -> None:
        super().__init__()
        self.key_cache: List[torch.Tensor] = []
        self.value_cache: List[torch.Tensor] = []
    ...
    def clip(self, layer_idx: int, attn_func, num_heads: int):
        ...
        w = math.ceil(1 / self.compression_ratio)
        key_cache_tmp = rearrange(key_cache, "b h s d -> (b h) d s")
        key_cache_tmp = torch.nn.functional.pad(key_cache_tmp, (w, w), mode='reflect')
        key_cache_mean = torch.nn.functional.avg_pool1d(key_cache_tmp, 2*w+1, stride=1)
        key_cache_mean = rearrange(key_cache_mean, "(b h) d s -> b h s d", h=num_heads)
        _, attn_weights = attn_func(key_cache - key_cache_mean)
        ...
        remaining_len = max_length - self.compressed_len - window_length - top_k
        merged_key = torch.zeros((comp_key.shape[0], comp_key.shape[1], remaining_len, comp_key.shape[-1]), device=comp_key.device, dtype=comp_key.dtype)
        merged_value = torch.zeros((comp_key.shape[0], comp_key.shape[1], remaining_len, comp_key.shape[-1]), device=comp_key.device, dtype=comp_key.dtype)
        merged_id = torch.zeros((comp_key.shape[0], comp_key.shape[1], remaining_len), device=comp_key.device, dtype=comp_key.dtype)
        merged_attn_score = torch.zeros((comp_key.shape[0], comp_key.shape[1], remaining_len), device=comp_key.device, dtype=comp_key.dtype)
        merged_attn_count = torch.zeros((comp_key.shape[0], comp_key.shape[1], remaining_len), device=comp_key.device, dtype=comp_key.dtype)
        for batch in range(comp_key.shape[0]):
            for head in range(comp_key.shape[1]):
                c_key = comp_key[batch, head, self.compressed_len:, :]
                separators = torch.linspace(0, c_key.shape[0] - 2, remaining_len - 1).round().unique().to(torch.int64)
                constrained_kmeans(c_key, separators)
                for i in range(len(separators) + 1):
                    ...
                    k_c = c_key[start:end, :].mean(dim=-2)
                    w_c = avg_attn_score[batch, head, start:end] / (end - start)
                    merged_attn_score[batch, head, i] = comp_attn_score[batch, head, start:end].sum(dim=-1) 
                    merged_attn_count[batch, head, i] = comp_attn_count[batch, head, start:end].sum(dim=-1) 
                    merged_key[batch, head, i, :] = k_c
                    merged_value[batch, head, i, :] = w_c @ comp_value[batch, head, start:end, :] * (end - start)
                    merged_id[batch, head, i] = (start + end) // 2
        self.compressed_len += remaining_len + top_k
