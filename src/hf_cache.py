...
def constrained_kmeans(values, separators):
    chunk_size = int(lvalues.shape[0] / (len(separators)+1)) + 1
...
class CentroidKV(Cache):
    ...
    def clip(self, layer_idx: int, attn_func, num_heads: int):
        key_cache = self.key_cache[layer_idx]
        cluster_size = math.ceil(1 / self.compression_ratio)
        kernel = torch.ones(1, 1, cluster_size*2 + 1, device=key_cache.device) / (cluster_size*2 + 1)
        key_cache_tmp = rearrange(key_cache, "b h s d -> b h d s")
        key_cache_mean = torch.nn.functional.conv1d(key_cache_tmp, kernel, padding=cluster_size, padding_mode='replicate')
        key_cache_mean = rearrange(key_cache_mean, "b h d s -> b h s d")
        ...
        remaining_len = max_length - self.compressed_length - window_length - top_k
        merged_key = torch.zeros((comp_key.shape[0], comp_key.shape[1], separators.shape[-1]+1, comp_key.shape[-1]), device=comp_key.device, dtype=comp_key.dtype)
        merged_value = torch.zeros((comp_key.shape[0], comp_key.shape[1], separators.shape[-1]+1, comp_key.shape[-1]), device=comp_key.device, dtype=comp_key.dtype)
        merged_id = torch.zeros((comp_key.shape[0], comp_key.shape[1], separators.shape[-1]+1), device=comp_key.device, dtype=comp_key.dtype)
        merged_attn_score = torch.zeros((comp_key.shape[0], comp_key.shape[1], separators.shape[-1]+1), device=comp_key.device, dtype=comp_key.dtype)
        ...
        for batch in range(comp_key.shape[0]):
            for head in range(comp_key.shape[1]):
                c_key = comp_key[batch, head, self.compressed_length:, :]
