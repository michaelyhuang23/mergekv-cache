...
class KNormCache(Cache):
    ...
    def update(
        ...
        #print('update called', max([cache.shape[-2] for cache in self.key_cache] + [0]), len(self.key_cache))
...
class MergeKV(Cache):
    ...
    def update(
        ...
        max_length = self.compression_ratio * super().get_seq_length()
        ...
        proj_key = comp_key.norm(dim=-1)
        key_idx = (-proj_key).topk(max_length - self.window_length, -1).indices.sort().values
        key_idx = key_idx[..., None].repeat(1, 1, 1, key_cache.shape[-1])
